...
def policy_to_vec(policy,is_mixed=True):
    policy_vec = np.asarray([policy['when_sees_apple']['eat'],
                  ...
                  policy['when_sees_green_player']['punish'],
                  policy['when_sees_normal_player']['punish']
                  ])
    if not is_mixed:
...
def state_to_vec(state):
    ...
    for player in state:
        player_obs = []
        state_evaluations = {k:f(player) for k,f in state_evals.items()}
        play_pos = (math.floor(player['x']/config.TILE_SIZE),math.floor(player['y']/config.TILE_SIZE))

        if state_evaluations['when_sees_banana']:
            player_obs.append(min([int(math.hypot(o['tilePosition']['x']-play_pos[0],o['tilePosition']['y']-play_pos[1])) for o in player['surroundingInfos']['banana']]))
            
        else:
            player_obs.append(-1) # this is only because the tile_size is close to 40
        
        if state_evaluations['when_sees_apple']:
            player_obs.append(min([int(math.hypot(o['tilePosition']['x']-play_pos[0],o['tilePosition']['y']-play_pos[1])) for o in player['surroundingInfos']['apple']]))
        else:
            player_obs.append(-1) # this is only because the tile_size is close to 40
        
        if state_evaluations['when_sees_green_player']:
            player_obs.append(min([int(math.hypot(o['tilePosition']['x']-play_pos[0],o['tilePosition']['y']-play_pos[1])) for o in player['surroundingInfos']['player'] if o['poisoned_player']]))
        else:
            player_obs.append(-1)
        
        if state_evaluations['when_sees_normal_player']:
            player_obs.append(min([int(math.hypot(o['tilePosition']['x']-play_pos[0],o['tilePosition']['y']-play_pos[1])) for o in player['surroundingInfos']['player'] if not o['poisoned_player']]))
        else:
            player_obs.append(-1)
        state_vec.append(player_obs)
        #player_obs.append(1)
    return state_vec
...
def handle_player_sprites(state,action_vec_policy_ma):
    actions = []
    visit_locations = []
    try:
        for idx,player in enumerate(state):
            player_visits = []
            action_vec_policy = action_vec_policy_ma[idx]
            #actions.append(control_player(player,player_policy))
            #action_vec_policy = policy_to_vec(player_policy,is_mixed=False)
            actions.append(action_vec_policy)
            state_evaluations = {k:f(player) for k,f in state_evals.items()}
            if action_vec_policy[0]==1 and state_evaluations['when_sees_apple']:
                player_visits.append(player['surroundingInfos']['apple'][0]['tilePosition'])
            if action_vec_policy[1]==1 and state_evaluations['when_sees_banana']:
                player_visits.append(player['surroundingInfos']['banana'][0]['tilePosition'])
            
            if len(player_visits) > 0:
                visit_locations.append(random.choice(player_visits))
            else:
                visit_locations.append(None)
    except IndexError:
        print('actions',action_vec_policy)
        raise

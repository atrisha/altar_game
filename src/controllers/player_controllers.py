...
player_policy_function = {
    'when_sees_normal_player': {'punish':0.33, 'avoid':0.33, 'do_nothing':0.34},
    'when_sees_green_player': {'punish':0.33, 'avoid':0.33, 'do_nothing':0.34},
    'when_sees_apple': {'eat':0.5, 'avoid':0.5},
    'when_sees_banana': {'eat':0.5, 'avoid':0.5},
    'when_turned_green':{'explore':0.5, 'visit_altar':0.13,'visit_fountain':0.12,'visit_houses':0.13,'visit_trees':0.11},
    'when_normal':{'explore':0.5, 'visit_altar':0.13,'visit_fountain':0.12,'visit_houses':0.13,'visit_trees':0.11}
    }
...
state_importance = {k:1/len(player_policy_function) for k in player_policy_function.keys()}

state_importance =  {'when_sees_normal_player': 0.15,    'when_sees_green_player': 0.15,    'when_sees_apple': 0.2,    'when_sees_banana': 0.2,    'when_turned_green': 0.15,    'when_normal': 0.15}
...
def surrounding_info_to_sentence(player):
    sentence_descr = ['I see ']
    play_pos = (math.floor(player['x']/config.TILE_SIZE),math.floor(player['y']/config.TILE_SIZE))
    _suffix = lambda d :' and some immaterial objects '+str(d)+' meters away'
    postpend = None
    for k,v in player['surroundingInfos'].items():
        ...
        if k == 'immaterial':
            ...
            sentence_descr.append(k+' '+str(dist_to_obj)+' meters away,')
    if postpend is not None:
        sentence_descr.append(postpend)
    if player['punished']:
        sentence_descr.append('I got punishment')
    if player['poisoned']:
        sentence_descr.append('I turned green')
...
def control_player(player,policy_function=player_policy_function):
    '''
    ...
    for k,v in player_policy_function.items():
    ...
    for k,v in all_state_evals.items():
        ...
        if v:
            ...
            for act, act_probs in player_policy_function[k].items():
                action_obj = dict()
...
def create_observation_for_memory(state,actions,rewards,time_st):
    batch_data = []
    for idx,player in enumerate(state):
        ...
        player_data['action_taken'] = actions[idx]['action'] + ' ' + actions[idx]['action_attribute']['object'] if 'object' in actions[idx]['action_attribute'] else actions[idx]['action']
        player_data['reward_received'] = rewards[idx]
        batch_data.append(player_data)
    return batch_data

def insert_observation_data(state,actions,rewards,time_st):
    script_dir = os.path.dirname(os.path.abspath(__file__))
    db_path = os.path.join(script_dir, '../../data/database/episodes.db')
    conn = sqlite3.connect(db_path)
    cursor = conn.cursor()
    batch_data = create_observation_for_memory(state,actions,rewards,time_st)
    '''
    batch_data = [
        {"player_id": 1, 
    ...
    sql = """
    INSERT INTO player_observations (player_id, observation_time, observation, action_taken, reward_received) 
    VALUES (?, ?, ?, ?, ?)
    """
    
    to_insert = [(data['player_id'], data['observation_time'], data['observation'], data['action_taken'], data['reward_received']) for data in batch_data]
